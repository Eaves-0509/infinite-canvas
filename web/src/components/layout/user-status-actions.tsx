...
import { BookOpen, ExternalLink, Keyboard, Puzzle, Settings2 } from "lucide-react";
import { useTranslation } from "react-i18next";

import { AnimatedThemeToggler } from "@/components/ui/animated-theme-toggler";
import { VersionReleaseModal } from "@/components/layout/version-release-modal";
import { DOCS_URL, EAVES_API_URL } from "@/constant/env";
...
export function UserStatusActions({ showConfig = true, variant = "default", onOpenShortcuts, onOpenPlugins }: UserStatusActionsProps) {
    ...
    const externalLinkClass = "inline-flex h-7 shrink-0 items-center gap-1 rounded-md px-2 text-xs font-medium text-stone-600 transition-colors hover:bg-black/5 hover:text-stone-950 dark:text-stone-300 dark:hover:bg-white/10 dark:hover:text-white";
    ...
    return (
        <div className="inline-flex shrink-0 items-center gap-1">
            ...
            <a href={EAVES_API_URL} target="_blank" rel="noopener noreferrer" className={externalLinkClass} style={iconStyle}>
                <ExternalLink className="size-3.5" />
                <span>{t("topNav.gateway")}</span>
            </a>
            <a href={DOCS_URL} target="_blank" rel="noopener noreferrer" className={externalLinkClass} style={iconStyle}>
