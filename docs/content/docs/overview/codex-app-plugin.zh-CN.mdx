...
```text
请从 https://https://github.com/Eaves-0509/infinite-canvas.git 安装 Infinite Canvas Codex 插件。
...
插件会启动本地 Canvas Agent，读取连接地址和 token，然后在右侧打开 `https://aihb.eaves.chat/` 并自动新建、连接画布。只有明确要求使用本地项目时，插件才会启动本地前端。
